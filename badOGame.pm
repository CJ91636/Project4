...
player playerO
 partyO, [sendO], [reqO], [begin], [ignoreO], [replyO], [endO], [endTurnO]
endplayer

player playerR
 partyR, [sendR], [reqR], [ignoreR], [replyR]
endplayer
...
module partyO
	...
	// sends providing there are still commitments left, the other party hasn't ignored the previous turn, and the party isn't TTPing
        [sendO] turnO=1 & sentBitsO<(N+1) & recdBitsO=sentBitsO  & !(contactO=1 & resultO=0) -> (sentBitsO'=sentBitsO+1);
...
module partyR
	...
	//[reqR] turnR=1 & !(contactR=1) & recdBitsR<=N & !(recdBitsR+1=sentBitsR) & (recdBitsR>=1 | !(resultO=1))-> (contactR'=1);
	[reqR] turnR=1 & !(contactR=1) & recdBitsR<=N & (recdBitsR>=1 | !(resultO=0)) -> (contactR'=1);
        [replyR] sendR=1 ->(resultR'=responseR) & (turnR'=2);
	...
	[ignoreR] false & turnR=1 & !(contactR=1 & resultR=0) -> (turnR'=0);
